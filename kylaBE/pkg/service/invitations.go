...
func FromProto(invitation *pb.Invitation) *Invitation {
	roles := pq.StringArray{}
	if invitation.RoleIds != nil {
		roles = append(roles, invitation.RoleIds...)
	}
	// Ensure the ID is parsed correctly
	if invitation.Id == "" {
		invitation.Id = uuid.New().String()
	}
	invitedBy, err := uuid.Parse(invitation.InvitedBy)
	if err != nil {
		invitedBy = uuid.Nil // Default to nil if parsing fails
	}
	orgID, err := uuid.Parse(invitation.OrganisationId)
	if err != nil {
	...
	i := &Invitation{
		...
		InvitedBy:      invitedBy,
		Type:           InvitationType(invitation.Type),
	...
	if invitation.BranchId != "" {
		branchID, err := uuid.Parse(invitation.BranchId)
		if err != nil {
			branchID = uuid.Nil
		}
		i.BranchID = branchID
	...
	if invitation.DepartmentId != "" {
		departmentID, err := uuid.Parse(invitation.DepartmentId)
		if err != nil {
	...
	if invitation.UserId != "" {
		userID, err := uuid.Parse(invitation.UserId)
		if err != nil {
			userID = uuid.Nil
		}
		i.UserID = userID
