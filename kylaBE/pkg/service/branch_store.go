...
// BranchStore is an interface for storing and retrieving branches

// BranchStore represents a store for managing branches
...
// Save saves a branch to the database
...
// FindByID retrieves a branch by its ID
...
func (bs *BranchStore) FindByOwner(scope *OpScope) ([]*Branch, error) {
	var branches []*Branch
	err := bs.DB.Transaction(func(tx *gorm.DB) error {
...
func (bs *BranchStore) FindDefaultBranch(scope *OpScope) (*Branch, error) {
	var branch Branch
	err := bs.DB.Transaction(func(tx *gorm.DB) error {
...
// Delete deletes a branch from the database
