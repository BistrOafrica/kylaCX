...
func (store *AppStore) FindAppByID(id string, scope *authctx.OpScope) (*App, error) {
	var app App
	err := store.db.Transaction(func(tx *gorm.DB) error {
...
func (store *AppStore) FindAppByName(name string, scope *authctx.OpScope) (*App, error) {
	var app App
	err := store.db.Transaction(func(tx *gorm.DB) error {
...
func (store *AppStore) FindAllAppsByOrganisationID(organisationID uuid.UUID, workspaceID *string) ([]*App, error) {
	var apps []*App
	query := store.db.Where("organisation_id = ?", organisationID)
...
// FindAll reads all console apps.
